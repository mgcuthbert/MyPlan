...
(async () => {
  ...
  [Controls.backgroundColor, Controls.fontColor].forEach((Element) => {
...
function loadConfigs(hostname: string): void {
  configStorage.get((configs) => {
    ...
    if (pageConfig) {
      ...
      Util.getInput(Controls.backgroundColor).value = pageConfig.customBackground
        ? pageConfig.customBackground
        : '#FFFFFF';
...
function saveConfig(hostname: string, saveConfigButton = false): void {
  configStorage.get((configs) => {
